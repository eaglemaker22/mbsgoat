...
async function fetchAndUpdateMarketData() {
  ...
  try {
    ...
    if (data?.US10Y) {
      ...
      updateChangeTextAndColor('us10yChange', isNaN(c) ? "--" : c.toFixed(3), true); // Update change (inverted)
    }
    ...
    if (data?.US30Y) {
      ...
      updateChangeTextAndColor('us30yChange', isNaN(c) ? "--" : c.toFixed(3), true); // Update change (inverted)
    }
  ...
  try {
    ...
    function updateRateRow(prefix, rateData) {
      ...
      if (rateData.daily_change !== undefined && rateData.daily_change !== null) {
        ...
        updateChangeTextAndColor(dailyChangeElementId, dailyChangeNumeric, true); // isInverted = true for rates
      } else {
...
async function fetchAndUpdateBondData() {
    ...
    try {
        ...
        // REMOVED: Timestamp update for bondLastUpdated
        // updateTextElement('bondLastUpdated', `Last Updated: ${data.last_updated ? new Date(data.last_updated.replace(' ', 'T')).toLocaleString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true, timeZone: 'America/Los_Angeles', timeZoneName: 'short' }) : '--'}`);
        ...
        bondInstruments.forEach(instrumentKey => {
            ...
            if (instrumentData) {
                ...
                // REMOVED: updateTextElement(`${tableIdPrefix}Updated`, formattedTimestampForTable);
                ...
                // REMOVED: updateTextElement(`${tableIdPrefix}Updated`, '--');
            }
        });

    } catch (err) {
        console.error("Bond data fetch error:", err);
        // REMOVED: updateTextElement('bondLastUpdated', `Last Updated: Error`);
